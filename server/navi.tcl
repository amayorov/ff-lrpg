...
namespace eval ::tcl::mathfunc {
    ...
    proc limit {x} {
	set y [expr {$x-2*pi()*floor($x/2/pi())}]
	return [expr {$y>pi()?$y-2*pi():$y}]
    }
}
...
namespace eval navi {
	...
	return [format "%.3f %.3f" $abs_speed $angle]
    }
...
# возвращает угол относительно "неподвижных звёзд"
	...
	set angle [expr {limit(atan2($y,$x)-$a1)}]
	set distance [expr {hypot($y,$x)}]
	set relangle [expr {limit($a2-$a1)}]
	...
	foreach obj [array names objects] {
	    if {! ($obj eq $shipname) } {
		set c [format "%.1f %.3f %.3f" {*}[rel_coords $shipname $obj]]
